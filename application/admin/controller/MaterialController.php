...
class MaterialController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $list = $this->materialService->indexService($param);
        return json(['code'=> 200, 'msg'=> '成功', 'data'=> $list]);
    }

    public function read($id)
    {
        $res = $this->materialService->readService($id);
        if($res['status']) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $res = $this->materialService->updateService($param, $id);
        if($res['status']) {
            return json(['code'=> 200, 'msg'=> $res['msg'], 'data'=> $res['data']]);
        }else{
            return json(['code'=> 401, 'msg'=> $res['msg']]);
    ...
    {
        $res = $this->materialService->deleteService($id);
        if($res['status']) {
