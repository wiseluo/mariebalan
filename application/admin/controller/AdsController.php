...
use think\Request;
use app\admin\service\AdsService;
...
class AdsController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $list = $this->adsService->indexService($param);
        return json(['code'=> 200, 'msg'=> '成功', 'data'=> $list]);
    }

    public function read($id)
    {
        $res = $this->adsService->readService($id);
        if($res['status']) {
    ...
    {
        $param = $request->param();
        $validate = validate('AdsValidate');
        if(!$validate->scene('save')->check($param)) {
            return json(['code'=> 401, 'type'=> 'save', 'msg'=> $validate->getError()]);
        }
        $param['url'] = $request->param('url', '#');
        $res = $this->adsService->saveService($param);
        if($res['status']) {
            return json(['code'=> 200, 'msg'=> $res['msg'], 'data'=> $res['data']]);
        }else{
            return json(['code'=> 401, 'msg'=> $res['msg']]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $param['url'] = $request->param('url', '#');
        $res = $this->adsService->updateService($param, $id);
        if($res['status']) {
            return json(['code'=> 200, 'msg'=> $res['msg'], 'data'=> $res['data']]);
        }else{
            return json(['code'=> 401, 'msg'=> $res['msg']]);
    ...
    {
        $res = $this->adsService->deleteService($id);
        if($res['status']) {
